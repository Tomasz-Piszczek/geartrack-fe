...
const CuttingTab: React.FC = () => {
  const { state, dispatch } = useQuote();
  const data = state.cutting;

  const handleFieldChange = (field: string, value: number) => {
    dispatch({ type: 'SET_TAB_FIELD', tab: 'cutting', field: field as any, value });
  };
  ...
  const handleMarginPlnChange = (value: number) => {
    ...
    dispatch({ type: 'SET_TAB_FIELD', tab: 'cutting', field: 'marginPln', value });
    dispatch({ type: 'SET_TAB_FIELD', tab: 'cutting', field: 'marginPercent', value: marginPercent });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-t pt-4">
        <h4 className="text-lg font-medium text-white mb-4">Koszt Wycinania</h4>
